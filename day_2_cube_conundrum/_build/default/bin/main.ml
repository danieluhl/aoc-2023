...
let extract_color str = 
  try
    let _ = Str.search_forward (Str.regexp "\\b\\(red\\|green\\|blue\\)\\b") str 0 in
...
let is_valid_draw draw =
  ...
  | Some (x) when x = "blue" -> Printf.printf "%s\n" x; (extract_int draw) < 15; 
  | Some (x) when x = "green" -> Printf.printf "%s\n" x; (extract_int draw) < 14
  | Some (x) when x = "red" -> Printf.printf "%s\n" x; (extract_int draw) < 13
  | _ -> false
...
let rec read_lines file_input_channel lines = 
  ...
  with
  | End_of_file -> close_in file_input_channel; List.rev lines
  | _ -> close_in file_input_channel; raise (Failure "Error reading file")
