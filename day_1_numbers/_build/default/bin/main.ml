...
let get_left_number line =
  List.fold search_num_list ~init:{ ch = ""; i = Int.max_value } ~f:(fun acc x ->
    let left_ch_i =
...
let get_right_number line =
  List.fold search_num_list ~init:{ ch = "0"; i = 0 } ~f:(fun acc x ->
    let right_ch_i =
...
let () =
  let lines = Day_1_numbers.Read.read_file "input.txt" in
  let result =
    List.fold
      ~init:0
      ~f:(fun acc line ->
        ...
        then (
          Printf.printf "\n+%s" left_ch;
          acc + int_of_string left_ch)
        else (
          Printf.printf "\n+%s%s" left_ch right_ch;
