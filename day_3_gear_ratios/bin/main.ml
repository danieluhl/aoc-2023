...
let lines = Day_3_gear_ratios.Read.read_file "input.txt"
let matrix = Array.of_list_map lines ~f:(fun line -> Array.of_list (String.to_list line))
...
let rec get_sum matrix x y num sum has_symbol =
  ...
  then (
    ...
    | _ ->
      (try
         (* go to the next row *)
         get_sum matrix (y + 1) 0 (num ^ String.make 1 ch) sum next_has_symbol
       with
       | _ -> sum + int_of_string (num ^ String.make 1 ch)))
  else (
    let next_sum = if String.length num > 0 then sum + int_of_string num else sum in
    try (* try going to the next column *)
        get_sum matrix y (x + 1) "" next_sum false with
    | _ ->
      (try (* go to the next row *)
           get_sum matrix (y + 1) 0 "" next_sum false with
       | _ -> next_sum))
;;
